...
export default function ExpHUD() {
  ...
  const expData = useMemo(() => {
    ...
    if (
      typeof anyMe.expInLevel === "number" &&
      typeof anyMe.expToNext === "number" &&
      anyMe.expToNext > 0
    ) {
      return {
        level,
  ...
  return (
    <div className="pointer-events-none absolute bottom-4 left-1/2 z-40 w-[420px] max-w-[92vw] -translate-x-1/2 rounded-xl border border-cyan-400/40 bg-[rgba(3,9,14,0.75)] px-4 py-3 shadow-[0_0_20px_rgba(0,255,255,0.15)] backdrop-blur">
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-7 min-w-7 items-center justify-center rounded-full bg-cyan-500/20 px-2 font-semibold text-cyan-200">
            Lv {level}
          </span>
          <span className="text-sm text-slate-300">
            EXP:{" "}
            <span className="font-semibold text-white">
              {Math.floor(currentExp).toLocaleString()}
            </span>
            {requiredExp && (
              <>
      ...
      {/* Progress bar */}
      {progress !== null ? (
        <div className="h-2 w-full overflow-hidden rounded bg-slate-700/60">
          <div
        ...
        <div className="h-[6px] w-full overflow-hidden rounded bg-slate-700/60">
          <div className="h-full w-1/3 animate-pulse rounded bg-cyan-400/60" />
        </div>
      )}
    </div>
  );
